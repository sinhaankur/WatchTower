...
- **`desktop/dist/` and `desktop/python-bundle/` can be STALE.** Both are git-ignored build artifacts that are NOT rebuilt automatically. Launching `desktop/dist/mac-arm64/WatchTower.app` directly can run an old build whose bundled `watchtower` code + alembic migrations are behind your DB — it crashes on startup with `alembic ... Can't locate revision '<rev>'` because the DB was upgraded past the bundle's last migration. This bit a real user on 1.16.3 (a leftover 1.14.4 bundle). Always install the **released DMG** to test the packaged app, never the repo's `desktop/dist/*`. `preflight.sh` now refuses to pack when `desktop/python-bundle` is stale, and `verify-release.sh` asserts the released bundle's version + migration count match — but those only guard the release path, not someone double-clicking an old local `.app`.
