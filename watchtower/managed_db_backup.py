...
# Engines where the restore path is implemented. Redis backup works but
# Redis-in-place-restore requires stopping the pod + replacing
# `/data/dump.rdb` + restarting — a qualitatively different operation
# than the dump-tool engines, and one that risks silent data corruption
# if done wrong. v1 explicitly rejects Redis restore with a clear
# "do this manually" message; v2 will automate it.
ENGINE_RESTORE_SUPPORTED = {"postgres", "mysql", "mariadb", "mongodb"}
...
def run_backup(spec: BackupSpec) -> int:
    ...
    if spec.engine == "postgres":
        ...
        # Redis doesn't have a "dump command" like the others — the
        # standard approach is to ask the server to write its RDB
        # snapshot, then copy that file out. `redis-cli --rdb <file>`
        # is the cleanest one-shot: triggers a SYNC, streams the RDB
        # response straight to the file. We run a transient redis
        # client container that has redis-cli available.
        #
        # AUTH happens via -a; the password is auto-generated by us
        # and stored encrypted, so leaking it through `ps` inside the
        # container is fine (no other tenants).
        #
        # Subtle: --rdb requires the server's `replication` permission
        # (or `+@all` for v6+ ACL). The official redis image's default
        # `requirepass`-based AUTH grants this; if the operator
        # post-creation locks the ACL down via WatchTower's terminal,
        # they'd need to grant the SYNC permission themselves.
        cmd = [bin_, "run", "--rm",
               "--network", "host",
               "-v", f"{host_dir}:/backup",
               spec.image,
               "sh", "-c",
               f"redis-cli -h {spec.primary_host} -p {spec.primary_port} "
               f"-a '{spec.db_password}' --no-auth-warning "
               f"--rdb /backup/{out_filename}"]
    else:
        raise BackupError(
...
def run_restore(spec: RestoreSpec) -> None:
    ...
    if spec.engine == "postgres":
        ...
        # Redis doesn't have a "restore from RDB" command at runtime —
        # the server only loads its RDB at startup. Approach:
        #   1. `FLUSHALL` so the live data is gone (the destructive
        #      part — same semantics as Postgres' --clean).
        #   2. Replay the RDB by piping it back to the live server as
        #      a series of RESTORE commands. We can't do that directly
        #      from an RDB file without a parser, so v1 takes the
        #      simpler path: use `redis-cli --pipe` with an RDB-to-
        #      protocol converter, OR — simpler still for v1 — emit
        #      a clear "not yet supported, copy dump.rdb manually
        #      and restart the pod" error.
        #
        # For v1 we choose the latter explicit-failure path: backup
        # works (lets operators capture state for safekeeping); restore
        # tells them what to do. Loading an RDB at runtime is a
        # qualitatively different operation than the other engines'
        # restore semantics, and getting it wrong silently corrupts
        # data. v2 will add a proper restore that stops the pod,
        # replaces dump.rdb in the volume, and restarts.
        raise BackupError(
            "Redis backups can be CREATED but in-place RESTORE is not yet "
            "supported. The .rdb file is on disk at the backup path — to "
            "restore manually: stop the pod, replace /data/dump.rdb in the "
            "Podman volume with the backup file, restart the pod. Automatic "
            "restore lands in a follow-up release."
        )
    else:
        raise BackupError(
